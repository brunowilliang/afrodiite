 ...
 */
export function useOnboardingStep(totalSteps: number, completedSteps: boolean[]): {
  currentStep: number;
  stepsState: StepState[];
} {
  const normalizedCompleted = useMemo(() => {
    const arr: boolean[] = Array.from({ length: totalSteps }, (_, i) => !!completedSteps[i]);
    return arr;
  ...
  const stepsState = useMemo<StepState[]>(() => {
    return normalizedCompleted.map((isCompleted, idx) => {
      ...
      const color: StepColor = isCompleted ? 'success' : isActive ? 'primary' : 'default';
      return {
        index: idx,
        isCompleted,
        isActive,
